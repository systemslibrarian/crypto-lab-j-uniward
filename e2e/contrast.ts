 ...
 *    `color-mix()`. This lab paints the hero's "why it matters" card and the
 *    image-load progress panel from `color-mix()` backgrounds and slides a
 *    `linear-gradient` across the progress bar, so axe files those nodes under
 *    `incomplete` rather than measuring them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The hero subtitle here
 *    renders at `opacity:.85`, the Quick Demo button drops to `.7` while it
 *    loads, and the STC schematic dims its unselected carrier cells to `.35`.
 *    Note this lab collapses its animations to 0.01ms under reduced motion,
 *    which preserves end states — including the button spinner and the sliding
 *    progress fill, both of which loop indefinitely otherwise.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every `.panel`
 *    here is `overflow: hidden` around a `.panel-body` that is `overflow-y:
 *    auto`, and the STC stage and histogram wrapper are `overflow-x: auto`.
 *    Content scrolled past those boxes is not dimmed or partly drawn — it is
 ...
 *    Skip it, and rely on the content that IS in view, which is measured for
 *    real.
 *
 ...
 *    is load-bearing here, not a spare part: the STC / Viterbi schematic
 *    (`#stc-svg`) draws each carrier coefficient as a `<rect>` filled from a
 *    cost ramp and then writes that coefficient's cost on top as a FOLLOWING
 *    `<text>`. The label's backdrop is therefore a preceding sibling shape, not
 *    an ancestor. An ancestor-only walk would composite those labels onto the
 *    svg's own transparent background — i.e. onto `.stc-stage` — and report a
 *    ratio nothing on screen has. So for SVG text, any earlier sibling shape
 *    whose box contains the text box is composited first.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A driven scan here walks three panels, the capacity table, the embedding
     * summary grid, the three-way exposure bars, the per-subband cost table and
     * the eight-cell STC schematic, and every one of those nodes re-walks the
     * same handful of ancestors up to `<body>`. Without the caches the pass
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has no <details>; what it does have is a
      // `.hidden { display: none !important }` utility toggled on nine panels
      // (the load-progress card, capacity warning, rate warning, embed status,
      // embed summary, post-embed strip, extract output, mechanism panel and
      // analysis explainer) and the class-toggled `.tab-pane.active` panes.
      // `checkVisibility()` catches every one of those; the gate drives each
      // panel into its revealed state and scans it there instead of unhiding
      // anything.
     ...
     *
     * This lab does have an SVG figure — the STC / Viterbi schematic — and it
     * builds it with `createElementNS` rather than string interpolation, so
